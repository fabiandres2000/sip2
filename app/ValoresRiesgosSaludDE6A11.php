...
class ValoresRiesgosSaludDE6A11 extends Model
{
    protected $table = 'valores_riesgos_salud_de6a11';
    protected $fillable = [
        'id_hogar',
    ...
    {
        $hay = DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de6a11')
        ->where('valores_riesgos_salud_de6a11.id_inte', $id_inte)
        ->where('valores_riesgos_salud_de6a11.estado', 'Activo')
        ->count();

        if($hay == 0){
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de6a11')->Insert([
                'id_hogar' => $id_hogar,
    ...
    {
        if ($opcion == "EnIn") {
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de6a11')->where('id_inte', $id_inte)->update([
    ...
    {
        return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de6a11')
        ->where('valores_riesgos_salud_de6a11.id_hogar', $id_hogar)
        ->where('valores_riesgos_salud_de6a11.estado', 'Activo')
        ->get();
    }
