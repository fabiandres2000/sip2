...
class ValoresRiesgosSaludDE60 extends Model
{
    protected $table = 'valores_riesgos_salud_de60';
    protected $fillable = [
        'id_hogar',
    ...
    {
        $hay = DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de60')
        ->where('valores_riesgos_salud_de60.id_inte', $id_inte)
        ->where('valores_riesgos_salud_de60.opci', $opci)
        ->where('valores_riesgos_salud_de60.estado', 'Activo')
        ->count();

        if($hay == 0){
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de60')->Insert([
                'id_hogar' => $id_hogar,
    ...
    {
        if ($opcion == "EnIn") {
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de60')->where('id_inte', $id_inte)->where('opci', $opci_crs)->update([
    ...
    {
        return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de60')
        ->where('valores_riesgos_salud_de60.id_hogar', $id_hogar)
        ->where('valores_riesgos_salud_de60.estado', 'Activo')
        ->get();
    }
