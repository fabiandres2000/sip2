...
class ValoresRiesgosSaludDE29A59 extends Model
{
    protected $table = 'valores_riesgos_salud_de29a59';
    protected $fillable = [
        'id_hogar',
    ...
    {
        $hay = DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de29a59')
        ->where('valores_riesgos_salud_de29a59.id_inte', $id_inte)
        ->where('valores_riesgos_salud_de29a59.opci', $opci)
        ->where('valores_riesgos_salud_de29a59.estado', 'Activo')
        ->count();

        if($hay == 0){
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de29a59')->Insert([
                'id_hogar' => $id_hogar,
    ...
    {
        if ($opcion == "EnIn") {
            return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de29a59')->where('id_inte', $id_inte)->where('opci', $opci_crs)->update([
    ...
    {
        return DB::connection('mysql')->table($alias . '.valores_riesgos_salud_de29a59')
        ->where('valores_riesgos_salud_de29a59.id_hogar', $id_hogar)
        ->where('valores_riesgos_salud_de29a59.estado', 'Activo')
        ->get();
    }
